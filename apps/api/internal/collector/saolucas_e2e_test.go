...
func TestSaoLucasCollector_LiveE2E(t *testing.T) {
	c := collector.NewSaoLucasCollector(nil, 15*time.Second)
	...
	if err != nil {
		t.Fatalf("falha ao coletar dados reais do Hospital São Lucas da PUCRS (portal ou layout alterado): %v", err)
	}

	if len(jobs) == 0 {
		t.Fatalf("nenhuma vaga foi extraída do portal real do Hospital São Lucas (possível mudança de estrutura)")
	}
	...
	t.Logf("Sucesso E2E Live: %d vagas extraídas do Hospital São Lucas. Primeira vaga: [%s] %s (%s)",
		len(jobs), first.ExternalID, first.Title, first.City)
}

func TestSaoLucasCollector_LiveE2E_Enfermagem(t *testing.T) {
	c := collector.NewSaoLucasCollector(nil, 15*time.Second)
	...
	t.Logf("=== VAGAS DE ENFERMAGEM ENCONTRADAS NO HOSPITAL SÃO LUCAS (%d) ===", len(jobs))
	for i, j := range jobs {
