...
func TestMoinhosCollector_LiveE2E(t *testing.T) {
	c := collector.NewMoinhosCollector(nil, 15*time.Second)
	...
	if err != nil {
		t.Fatalf("falha ao coletar dados reais do Hospital Moinhos de Vento (portal ou layout alterado): %v", err)
	}

	if len(jobs) == 0 {
		t.Fatalf("nenhuma vaga foi extraída do portal real do Hospital Moinhos de Vento (possível mudança de estrutura)")
	}
	...
	t.Logf("Sucesso E2E Live: %d vagas extraídas do Moinhos de Vento. Primeira vaga: [%s] %s (%s)",
		len(jobs), first.ExternalID, first.Title, first.City)
}

func TestMoinhosCollector_LiveE2E_Enfermagem(t *testing.T) {
	c := collector.NewMoinhosCollector(nil, 15*time.Second)
	...
	t.Logf("=== VAGAS DE ENFERMAGEM ENCONTRADAS NO MOINHOS DE VENTO (%d) ===", len(jobs))
	for i, j := range jobs {
