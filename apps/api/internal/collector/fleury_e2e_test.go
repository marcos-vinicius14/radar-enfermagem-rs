...
func TestFleuryCollector_LiveE2E(t *testing.T) {
	c := collector.NewFleuryCollector(nil, 15*time.Second)
	...
	if err != nil {
		t.Fatalf("falha ao coletar dados reais do Grupo Fleury / Weinmann (portal ou layout alterado): %v", err)
	}

	if len(jobs) == 0 {
		t.Fatalf("nenhuma vaga foi extraída do portal real do Grupo Fleury / Weinmann (possível mudança de estrutura)")
	}
	...
	t.Logf("Sucesso E2E Live: %d vagas extraídas do Grupo Fleury / Weinmann. Primeira vaga: [%s] %s (%s)",
		len(jobs), first.ExternalID, first.Title, first.City)
}

func TestFleuryCollector_LiveE2E_Enfermagem(t *testing.T) {
	c := collector.NewFleuryCollector(nil, 15*time.Second)
	...
	t.Logf("=== VAGAS DE ENFERMAGEM ENCONTRADAS NO GRUPO FLEURY / WEINMANN (%d) ===", len(jobs))
	for i, j := range jobs {
