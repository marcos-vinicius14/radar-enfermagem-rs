...
func TestDivinaCollector_LiveE2E(t *testing.T) {
	c := collector.NewDivinaCollector(nil, 15*time.Second)
	...
	if err != nil {
		t.Fatalf("falha ao coletar dados reais da Divina Providência (portal ou layout alterado): %v", err)
	}

	if len(jobs) == 0 {
		t.Fatalf("nenhuma vaga foi extraída do portal real da Divina Providência (possível mudança de estrutura)")
	}
	...
	t.Logf("Sucesso E2E Live: %d vagas extraídas da Divina Providência. Primeira vaga: [%s] %s (%s)",
		len(jobs), first.ExternalID, first.Title, first.City)
}

func TestDivinaCollector_LiveE2E_Enfermagem(t *testing.T) {
	c := collector.NewDivinaCollector(nil, 15*time.Second)
	...
	t.Logf("=== VAGAS DE ENFERMAGEM ENCONTRADAS NA DIVINA PROVIDÊNCIA (%d) ===", len(jobs))
	for i, j := range jobs {
